...
Firebug.HTMLPanel.prototype = extend(Firebug.Panel,
{
    ...
    mutateText: function(target, parent, textValue)
    {
        ...
        if (!parentNodeBox)
        {
            if (FBTrace.DBG_HTML)   FBTrace.sysout("html.mutateText failed to update text, parent node 3dbox_1.1 does not exist");
            return;
        }
        ...
        else
        {
            ...
            if (!childBox)
            {
                if (FBTrace.DBG_HTML)   FBTrace.sysout("html.mutateText failed to update text, no child object 3dbox_1.1 found");
                return;
            }
    ...
    getChildObject: function(node, index, previousSibling)
    {
        ...
        if (Firebug.showTextNodesWithWhitespace)  // then the product is true to the node list
            return child;
        else
        {
